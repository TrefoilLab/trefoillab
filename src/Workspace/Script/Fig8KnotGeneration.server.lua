...
-- Bjørn Kjos-Hanssen: inspired by generateFig8Knot and calculus considerations
function generateTrefoilCalculus(points, radius)
	local pointsList = {}
	local step = (2 * math.pi) / points

	for i = 0, points - 1 do
		local t = i * step
		local x = (2 + math.sin(3 * t)) * math.cos(2 * t)
		local y = (2 + math.sin(3 * t)) * math.sin(2 * t)
		local z = - math.cos(3 * t)
...
local zOffset =	-50		-- origin z offset

local xOffset2 = 584			-- origin x offset
local yOffset2 = 100			-- origin y offset
local zOffset2 = -50		-- origin z offset
...
-- B K-H
local knot2 = generateTrefoilCalculus(points, radius)
